function [shell] = sym_Shell(par,bc,n_dof,path)
...
Us_filename = sprintf(['Us_don_%ddof_' bc '_cou_' nrg_cou '.mat'],n_dof); % filename contains number of dofs 
...
T_filename = sprintf(['T_don_%ddof_10ab%d%d_1000h%d_R%d%d' bc '.m'],n_dof,10*par.a,10*par.b,1000*par.h,par.Rx,par.Ry); % filename contains number of dofs 

if(exist(T_filename,'file') == 2)
    T_fun = str2func(T_filename(1:end-2));
    
    disp('T_fun loaded from file.')
else
    tic
    T = expand(du^2 + dv^2 + dw^2);
    T = int(T,x,0,a);
    T = 1/2*rho*h*int(T,y,0,b);
    T = subs(T,{rho a b h E Rx Ry nu W0(1,1)},{par.rho par.a par.b par.h par.E par.Rx par.Ry par.nu par.A11});
    t_T = toc
    
    T_fun = matlabFunction(T,'file','T_fun','Vars',{dq},'File',strcat(T_path,T_filename));
...
c_val = par.zeta*par.rho*par.h*par.a*par.b.*par.eig/2; % numerical values of the damping coefficients
c = c
F = simplify(a*b/8*F)

%% External Loading

Mh_w = floor(Mw/2+1); %(=1 for 9 dofs and =2 for 22 dofs)
Nh_w = floor(Nw/2+1);
...
for i = 1:n_dof
    dUs(i) = 4/(rho*h*a*b)*diff(Us,q(i));                         % CORRECT
...
rhs = [dq;Q-dUs]; % right-hand side of the EOM
...
Xi = [xi;dxi];
rhs_nd = [dxi;Q_nd-dUs_nd];

% Jacobian
for i = 1:2*n_dof
    J_nd(:,i) = diff(rhs_nd,Xi(i));
end
...
Q_filename = sprintf(['Q_don_%ddof_10ab%d%d_1000h%d_R%d%d_1000z11%d_' bc '.m'],n_dof,10*par.a,10*par.b,1000*par.h,par.Rx,par.Ry,1000*par.zeta(1));
dUs_filename = sprintf(['dUs_don_%ddof_10ab%d%d_1000h%d_R%d%d_' bc '_cou_' nrg_cou '.m'],n_dof,10*par.a,10*par.b,1000*par.h,par.Rx,par.Ry);
J_filename = sprintf(['J_don_%ddof_10ab%d%d_1000h%d_R%d%d_1000z11%d_' bc '_cou_' nrg_cou '.m'],n_dof,10*par.a,10*par.b,1000*par.h,par.Rx,par.Ry,1000*par.zeta(1));
...
Q_nd_filename = sprintf(['Q_don_nd_%ddof_10ab%d%d_1000h%d_R%d%d_1000z11%d_' bc '_cou_' nrg_cou '.m'],n_dof,10*par.a,10*par.b,1000*par.h,par.Rx,par.Ry,1000*par.zeta(1));
dUs_nd_filename = sprintf(['dUs_don_nd_%ddof_10ab%d%d_1000h%d_R%d%d_' bc '_cou_' nrg_cou '.m'],n_dof,10*par.a,10*par.b,1000*par.h,par.Rx,par.Ry);
J_nd_filename = sprintf(['J_don_nd_%ddof_10ab%d%d_1000h%d_R%d%d_1000z11%d_' bc '_cou_' nrg_cou '.m'],n_dof,10*par.a,10*par.b,1000*par.h,par.Rx,par.Ry,1000*par.zeta(1));
