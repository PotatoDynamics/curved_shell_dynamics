function [shell] = sym_shell_general(M,N,Mw,Nw,M0,N0,par,ansatz,bc,path)
% This was the initial function for performing the symbolic computations.
% It includes both Donnell's and Novozhilov's shell theory and two versions
% of the additional modal expansion terms for each. There is also a choice
% of different boundary-conditions discussed in Amabili's 2004 paper about
% plates.

shell = struct;

%% Paths
if strcmp(ansatz,'nov')
    Us_path = strcat(path,'Elastic_Forces\Novozhilov\'); % path to Elastic_Forces folder
    Q_path = strcat(path,'External_Forces\Novozhilov\');
    J_path = strcat(path,'Jacobians\Novozhilov\'); 
else
...
V = sym('v%d%d', [2*M 2*N]);
W = sym('w%d%d', [Mw Nw]);
W0 = sym('A%d%d', [M0 N0]);      % modal amplitudes (time-dependent)

dU = sym('du%d%d', [2*M 2*N]);
dV = sym('dv%d%d', [2*M 2*N]);
dW = sym('dw%d%d', [Mw Nw]);    % modal velocities

  % non-dimensionalized vectors of dofs and their derivatives

u = 0; v = 0; w = 0; w0 = 0;
i = 1;

% Different modal extensions are used for immovable/clamped and movable
% edges.
if strcmp(bc.type,'immovable') || strcmp(bc.type,'clamped')
    % modal extension for clamped/immovable edges
    n_dof = 2*M*N + Mw*Nw;
    q = sym('q%d', [n_dof 1]);
    ...
    for m = 1:2:Mw
        for n = 1:2:Nw
            w = w + W(m,n)*sin(m*pi*x/a)*sin(n*pi*y/b);
            q(i) = W(m,n);
            dq(i) = dW(m,n);
            i = i + 1;
        end
    ...
    for m = 1:M
        for n = 1:N
            u = u + U(2*m,n)*sin(2*m*pi*x/a)*sin(n*pi*y/b);
            v = v + V(m,2*n)*sin(m*pi*x/a)*sin(2*n*pi*y/b); 
    ...
    disp('Modal extension for immovable edges calculated.')
else
    ...
    % w33]'
    n_dof = 2*ceil(M/2)*ceil(N/2) + ceil(Mw/2)*ceil(Nw/2);
...
if strcmp(bc.type,'movable') && strcmp(bc.add,'full') 
    for m = 1:2:Mw
        for n = 1:2:Nw
            for k = 1:2:Nw
                for s = 1:2:Mw
...
elseif strcmp(bc.type,'movable') && strcmp(bc.add,'simplified') && strcmp(ansatz,'don') 
     for m = 1:2:Mw
        for n = 1:2:Nw
            for j = 1:2:N0
                for i = 1:2:M0
    ...
    disp('Additional SIMPLIFIED expansion terms (Donnell) for Boundary Conditions added.')
elseif strcmp(bc.type,'movable') && strcmp(bc.add,'simplified') && strcmp(ansatz,'nov') 
    for m = 1:2:Mw
        for n = 1:2:Nw
            for j = 1:2:N0
                for i = 1:2:M0
            ...
            uh = uh - 1/8*(((m*pi/a*(W(m,n)^2 + V(m,n)^2) + a/(m*pi)*U(m,n)^2/Rx^2 - 2*W(m,n)*U(m,n)/Rx - nu/8*a/(m*pi)*(n*pi/b)^2*U(m,n)^2) ...
                    +(-m*pi/a*(W(m,n)^2 - V(m,n)^2) - a/(m*pi)*U(m,n)^2/Rx^2 + 2*W(m,n)*U(m,n)/Ry + nu/8*a/(m*pi)*(n*pi/b)^2*U(m,n)^2 ...
                     + 1/4*m*pi/a*V(m,n)^2)*cos(2*n*pi*y/b))*sin(2*m*pi*x/a) - m*pi/a*W(m,n)*sin(n*pi*y/b)*u_ij);
            vh = vh - 1/8*(((n*pi/b*(W(m,n)^2 + U(m,n)^2) + b/(n*pi)*V(m,n)^2/Ry^2 - 2*W(m,n)*V(m,n)/Ry - nu/8*b/(n*pi)*(m*pi/a)^2*V(m,n)^2) ...
                    +(-n*pi/b*(W(m,n)^2 - U(m,n)^2) - b/(n*pi)*V(m,n)^2/Ry^2 + 2*W(m,n)*V(m,n)/Ry + nu/8*b/(n*pi)*(m*pi/a)^2*V(m,n)^2 ...
                     + 1/4*n*pi/b*U(m,n)^2)*cos(2*m*pi*x/a))*sin(2*n*pi*y/b) - n*pi/b*W(m,n)*sin(m*pi*x/a)*u_ij);
        end
    end
    disp('Additional SIMPLIFIED expansion terms (Novozhilov) for Boundary Conditions added.')
else
    uh = 0;
    vh = 0;
end
...
% energy stored at clamped edges
k = 1000;

if strcmp(bc.type,'clamped')
    U_R = 1/2*int(k*(subs(diff(w,x),x,0)^2 + subs(diff(w,x),x,a)^2),y,0,b) ...
           + 1/2*int(k*(subs(diff(w,y),y,0)^2 + subs(diff(w,y),y,b)^2),x,0,a);
    disp('Additional energy term for clamped edges added.')
else
    U_R = 0;
end
 
%% Functions for Plotting
% These functions are for calculating the displacements from the modal
% amplitudes.
...
% 
% u_plot_fun = matlabFunction(u_plot,'file','u_plot_fun.m','Vars',{q x y Rx Ry nu},'File',strcat(plot_path,'u_plot_fun.m'));
% v_plot_fun = matlabFunction(v_plot,'file','v_plot_fun.m','Vars',{q x y Rx Ry nu},'File',strcat(plot_path,'v_plot_fun.m'));
% w_plot_fun = matlabFunction(w_plot,'file','w_plot_fun.m','Vars',{q x y},'File',strcat(plot_path,'w_plot_fun.m'));

%% Strain-Displacement

% x and y have to be replaced by psi and theta for differentiation.
u = subs(u, [x y], [Rx*psi Ry*theta]);
v = subs(v, [x y], [Rx*psi Ry*theta]);
w = subs(w, [x y], [Rx*psi Ry*theta]);
w0 = subs(w0, [x y], [Rx*psi Ry*theta]);

if strcmp(ansatz,'nov')
    % Novozhilov
    eps_x0  = diff(u,psi)/Rx + w/Rx + 1/(2*Rx^2)*((diff(u,psi) + w)^2 + diff(v,psi)^2 + (diff(w,psi) - u)^2) + ...
            1/Rx^2*(diff(w0,psi)*(diff(w,psi) - u) + w0*(diff(u,psi) + w));
            
    eps_y0  = diff(v,theta)/Ry + w/Ry + 1/(2*Ry^2)*((diff(v,theta) + w)^2 + diff(u,theta)^2 + (diff(w,theta) - v)^2) + ...
             1/Ry^2*(diff(w0,theta)*(diff(w,theta) - v) + w0*(diff(v,theta) + w));

    gam_xy0 = diff(u,theta)/Ry + diff(v,psi)/Rx + 1/(Rx*Ry)*(diff(u,theta)*(diff(u,psi) + w + w0) + diff(v,psi)*...
             (diff(v,theta) + w + w0) + (diff(w,psi) - u)*(diff(w,theta) - v) + diff(w0,psi)*(diff(w,theta) - v) + ...
             diff(w0,theta)*(diff(w,psi) - u));

    k_x  = -diff(w,psi,2)/Rx^2 + w/(Rx*Ry) + diff(v,theta)/(Rx*Ry) + diff(u,psi)/Rx^2;

    k_y  = -diff(w,theta,2)/Ry^2 + w/(Rx*Ry) + diff(u,psi)/(Rx*Ry) + diff(v,theta)/Ry^2;

    k_xy = -2*diff(w,psi,theta)/(Rx*Ry) + diff(v,psi)/Rx*(-1/Rx + 1/Ry) + diff(u,theta)/Ry*(1/Rx - 1/Ry);
else
    % Donnell (much simpler)
    eps_x0 = diff(u,psi)/Rx + w/Rx + 1/2*(diff(w,psi)/Rx)^2 + diff(w,psi)/Rx*diff(w0,psi)/Rx;           % CORRECT

    eps_y0 = diff(v,theta)/Ry + w/Ry + 1/2*(diff(w,theta)/Ry)^2 + diff(w,theta)/Ry*diff(w0,theta)/Ry;   % CORRECT

    gam_xy0 = diff(u,theta)/Ry + diff(v,psi)/Rx + diff(w,psi)/Rx*diff(w,theta)/Ry + ...
              diff(w,psi)/Rx*diff(w0,theta)/Ry + diff(w0,psi)/Rx*diff(w,theta)/Ry;                      % CORRECT

    k_x = -diff(w,psi,2)/(Rx^2);            % CORRECT

    k_y = -diff(w,theta,2)/(Ry^2);          % CORRECT

    k_xy = -2*diff(w,psi,theta)/(Rx*Ry);    % CORRECT
end
...
eps_x0  = subs(eps_x0,  [psi theta], [x/Rx y/Ry]);
eps_y0  = subs(eps_y0,  [psi theta], [x/Rx y/Ry]);
gam_xy0 = subs(gam_xy0, [psi theta], [x/Rx y/Ry]);

k_x  = subs(k_x,  [psi theta], [x/Rx y/Ry]);
k_y  = subs(k_y,  [psi theta], [x/Rx y/Ry]);
k_xy = subs(k_xy, [psi theta], [x/Rx y/Ry]);

Us_filename = sprintf(['Us_%ddof_' bc.type '_' bc.add '_' ansatz '.mat'],n_dof); % filename contains number of dofs 
...
if(exist(Us_filename,'file') == 2)
    ...
    t1 = int(t1,y,0,b);
    t_x0 = toc
%     t1 = simplify(t1,'Seconds',10);         % CORRECT
    ...
    t2 = int(t2,x,0,a);
    t_y0 = toc
%     t2 = simplify(t2,'Seconds',10);         % CORRECT
    ...
    t_xy0 = toc
%     t3 = simplify(t3,'Seconds',10);         % CORRECT
    ...
    t_gam = toc
%     t4 = simplify(t4,'Seconds',10);         % CORRECT

    U_str = 1/2*E*h/(1 - nu^2)*(t1 + t2 + t3 + t4); % FIXED
    ...
    t5 = int(t5,x,0,a);
    t5 = int(t5,y,0,b);
%     t5 = simplify(t5,'Seconds',10);     % CORRECT

    t6 = expand(k_y^2);
    t6 = int(t6,x,0,a);
    t6 = int(t6,y,0,b);
%     t6 = simplify(t6,'Seconds',10);     % CORRECT

    t7 = expand(k_x*k_y);
    t7 = int(t7,x,0,a);
    t7 = 2*nu*int(t7,y,0,b);
%     t7 = simplify(t7,'Seconds',10);     % FIXED

    t8 = expand(k_xy^2);
    t8 = int(t8,x,0,a);
    t8 = (1 - nu)/2*int(t8,y,0,b);
%     t8 = simplify(t8,'Seconds',10);     % CORRECT
    t_ben = toc
 
    U_ben = 1/2*E*h^3/(12*(1 - nu^2))*(t5 + t6 + t7 + t8);  % FIXED
    ...
    t9 = int(t9,y,0,b);
    t_kx = toc
%     t9 = simplify(t9,'Seconds',10);     % CORRECT
    ...
    t10 = int(t10,x,0,a);
    t_ky = toc
%     t10 = simplify(t10,'Seconds',10);   % CORRECT

    t11 = expand(eps_x0*k_y);
    t11 = int(t11,x,0,a);
    t11 = nu*int(t11,y,0,b);
%     t11 = simplify(t11,'Seconds',10);   % CORRECT

    t12 = expand(eps_y0*k_x);
    t12 = int(t12,y,0,b);
    t12 = nu*int(t12,x,0,a);
%     t12 = simplify(t12,'Seconds',10);   % CORRECT

    t13 = expand(gam_xy0*k_xy);
    t13 = int(t13,x,0,a);
    t13 = (1 - nu)/2*int(t13,y,0,b);    % CORRECT
%     t13 = simplify(t13,'Seconds',10);
    t_cou = toc

    U_cou = 1/2*(1/Rx + 1/Ry)*E*h^3/(6*(1 - nu^2))*(t9 + t10 + t11 + t12 + t13);    % CORRECT
   
    Us = simplify(U_str + U_ben + U_cou,'Steps',15);
end
...
if strcmp(bc.type,'immovable') || strcmp(bc.type,'clamped')
    % modal damping for immovable/clamped edges
    c = sym('c%d',[n_dof 1]);
    ...
    for m = 1:M
        for n = 1:N
            F = F + C(2*m,n)*dU(2*m,n)^2 + C(m,2*n)*dV(m,2*n)^2;
            c(i) = C(2*m,n);
            c(i+1) = C(m,2*n);
            i = i + 2;
        end
...
% uniform pressure
% p = sym('p');           % normal pressure
% ome_p = sym('ome_p');   % frequency in [rad/s]

% W_p = 0;
% 
% for i = 1:Mh_w
%     for j = 1:Nh_w
%         W_p = W_p + W(2*i-1,2*j-1)/((2*i-1)*(2*j-1));
%     end
% end
% 
% W_p = 4*a*b/sym(pi)^2*p*cos(ome_p*t)*W_p;
...
% for i = 1:Mh_u
%     for j = 1:Nh_u
%         W_in = W_in + U(2*i-1,2*j-1)/(2*j-1);
%     end
% end
% 
% W_in = -4*b/pi*f_in*W_in;
...
for i = 1:n_dof
    dUs(i) = 1/(rho*h*a*b/4)*diff(Us + U_R,q(i));                   % CORRECT
...
Q_filename = sprintf(['Q_%ddof_' bc.type '_' bc.add '_' ansatz '.m'],n_dof);
dUs_filename = sprintf(['dUs_%ddof_' bc.type '_' bc.add '_' ansatz '.m'],n_dof);
J_filename = sprintf(['J_%ddof_' bc.type '_' bc.add '_' ansatz '.m'],n_dof);
...
Q_nd_filename = sprintf(['Q_nd_%ddof_10ab%d%d_1000h%d_R%d%d_1000z11%d_' bc.add '.m'],n_dof,10*par.a,10*par.b,1000*par.h,par.Rx,par.Ry,1000*par.zeta(1));
dUs_nd_filename = sprintf(['dUs_nd_%ddof_10ab%d%d_1000h%d_R%d%d_' bc.add '.m'],n_dof,10*par.a,10*par.b,1000*par.h,par.Rx,par.Ry);
J_nd_filename = sprintf(['J_nd_%ddof_10ab%d%d_1000h%d_R%d%d_1000z11%d_' bc.add '.m'],n_dof,10*par.a,10*par.b,1000*par.h,par.Rx,par.Ry,1000*par.zeta(1));
